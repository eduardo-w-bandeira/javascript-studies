...
const user = await User.findById(allUsers.findOne()._id);
console.log(user);
...
// populatedUsers.forEach(each_user => {
//     const postCount = each_user.posts.length;
//     const commentCount = each_user.posts.reduce((total, post) => total + post.comments.length, 0);
//     each_user.totalPostsAndComments = postCount + commentCount;
// });

// populatedUsers.sort((a, b) => b.totalPostsAndComments - a.totalPostsAndComments);
...
// const usersWithNoPostOrComment = populatedUsers.filter(
//     each_user => each_user.posts.length === 0 && each_user.comments.length === 0);
...
// const postsPopulatedWithComments = await Post.find().populate('comments').exec();
// const postsGroupedByYear = postsPopulatedWithComments.reduce((acc, post) => {
//     // Get the year from the createdAt date of the post
//     const publicationYear = post.createdAt.getFullYear();

//     // If the year key doesn't exist in the accumulator, create it
//     if (!acc[publicationYear]) {
//         acc[publicationYear] = [];
//     }

//     // Push the post to the corresponding year array
//     acc[publicationYear].push(post);
